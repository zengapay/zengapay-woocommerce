<?php
/**
 * Zengapay Payments Custom Post Type for the client follow up.
 */
function zengapay_setup_post_type() {
    $args = array(
        'public'    => true,
    ...
    register_post_type( 'zengapayments', $args );
}

add_action( 'init', 'zengapay_setup_post_type' );

// Disable Add new Posts.
function disable_new_posts() {
    // Hide sidebar link
    global $submenu;
    unset($submenu['edit.php?post_type=zengapayments'][10]);

    // Hide link on listing page
    if (isset($_GET['post_type']) && $_GET['post_type'] == 'zengapayments') {
...
// Add metabox
function zengapay_add_custom_box()
{
    $screens = ['zengapayments'];
    foreach ($screens as $screen) {
        add_meta_box(
            'zengapay_payments_box',           // Unique ID
            'Payment Information',  // Box title
            'zengapay_show_admin_boxes',  // Content callback, must be of type callable
            $screen                   // Post type
        );
    }
}
add_action('add_meta_boxes', 'zengapay_add_custom_box');

// Show admin side boxes
function zengapay_show_admin_boxes( $post )
{
    ?>
...
function zengapy_custom_columns_list($columns) {
...
add_filter( 'manage_zengapayments_posts_columns', 'zengapy_custom_columns_list' );
add_filter( 'manage_zengapayments_posts_custom_column', 'zengapay_add_custom_column_data',10 ,2 );

function zengapay_add_custom_column_data($column, $post_id) {
