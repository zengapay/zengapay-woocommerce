<?php
/**
 * Zengapay Gateway Class
 */

/**
 * Zengapay Gateway.
 *
 * Provides a Zengapay Payment Gateway.
 *
 * @class       WC_Zengapay_Gateway
 * @extends     WC_Payment_Gateway
 * @version     0.1.0
 */

if ( ! class_exists( 'WC_Zengapay_Gateway' ) && class_exists( 'WC_Payment_Gateway' ) ) {
	class WC_Zengapay_Gateway extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_thankyou_order_received_text', array( $this, 'zengapay_thank_you' ) );
		...
		protected function setup_properties() {
			...
			$this->payment_url = 'https://api.zengapay.com/v1/collections';
		...
		public function init_form_fields() {
			$this->form_fields = array(
				...
				'title' => array(
					'title' => __( 'Title', 'zengapay-pay-woo'),
					'type' => 'text',
					'default' => __( 'ZENGAPAY (Mobile Money Gateway)', 'zengapay-pay-woo'),
					'desc_tip' => true,
					'description' => __( 'This controls the title which the user sees during checkout.', 'zengapay-pay-woo')
				),
				'description' => array(
					'title' => __( 'Checkout Description', 'zengapay-pay-woo'),
					'type' => 'textarea',
					'default' => __( 'Pay using either your Mobile Phone on MTN Mobile Money or Airtel Money.', 'zengapay-pay-woo'),
					'desc_tip' => true,
					'description' => __( 'This controls the description which the user sees during checkout.', 'zengapay-pay-woo')
				),
				'instructions' => array(
				...
				'apiKey' => array(
					...
					'type' => 'text',
					'desc_tip' => true,
					'default' => sprintf( __( 'Obtain this from your ZENGAPAY Dashboard - %s', 'zengapay-pay-woo'), esc_url( 'https://dashboard.zengapay.com/settings/developer-settings' ) ),
				),
				'secretKey' => array(
					'title' => __( 'Secret Hash', 'zengapay-pay-woo'),
					'description' => sprintf( __( 'Obtain this from your ZENGAPAY Dashboard - %s', 'zengapay-pay-woo'), esc_url( 'https://dashboard.zengapay.com/settings/developer-settings' ) ),
					'type' => 'text',
					'desc_tip' => true,
					'default' => __( '', 'zengapay-pay-woo'),
				),
				'webhook_url' => array(
					'title'   => __( 'INCOMING WEBHOOK URL', 'zengapay-pay-woo' ),
		...
		public function zengapay_thank_you() {
		...
		public function change_payment_complete_order_status( $status, $order_id = 0, $order = false ) {
			if ( $order && 'zengapay_payment' === $order->get_payment_method() ) {
				$status = 'wc-invoiced';
			}
			return $status;
		}
		...
		public function is_valid_for_use() {
			if( ! in_array( get_woocommerce_currency(), array( 'UGX' ) ) ) {
				$this->msg = 'Zengapay does not support your store currency, set it to Ugandan Shillings UGX <a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=wc-settings&tab=general">here</a>';
				return false;    
			}
			return true;
		}
